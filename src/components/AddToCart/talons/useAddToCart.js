...
import defaultOperations from './addToCart.gql';
import { removeItemMutation, updateItemQuantityMutation } from './addToCartOptions.gql';
...
export const useAddToCart = props => {
    ...
    const isSupportedProductType = true;//isSupported(productType);
    ...
    const handleAddToCart = useCallback(
        //async formValues => {
        async (quantity, additionalParam = null) => {
            ...
            if (hasDeprecatedOperationProp) {
                ...
                if (!variables.product.selected_options) {
                    variables.product.selected_options = [];
                }

                if (additionalParam && Array.isArray(additionalParam) && additionalParam.length > 0) {
                    // Option 1: Replace the existing selected_options with bundle_selected_options
                    variables.product.selected_options = additionalParam.map((option) => option);
                    // Option 2: Append bundle_selected_options to existing selected_options
                    //variables.selected_options = [...variables.selected_options, ...product.bundle_selected_options];
                    // or
                    // variables.selected_options = variables.selected_options.concat(product.bundle_selected_options);
                }

                if (product.product_type === 'bundle' && Array.isArray(product.bundle_selected_options)) {
                    // Option 1: Replace the existing selected_options with bundle_selected_options
                    variables.product.selected_options = product.bundle_selected_options.map((option) => option);
                    // Option 2: Append bundle_selected_options to existing selected_options
                    //variables.selected_options = [...variables.selected_options, ...product.bundle_selected_options];
                    // or
                    // variables.selected_options = variables.selected_options.concat(product.bundle_selected_options);
    ...
    }, [cartId, dispatch, product, removeItemFromCart]);
    ...
    const handleUpdateItemQuantity = useCallback(
        ...
        [cartId, dispatch, product, updateItemQuantity]
    );
