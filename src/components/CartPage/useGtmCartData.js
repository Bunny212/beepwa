...
import { GET_GTM_VIEW_CART_ITEMS } from './gtmViewCartItems.qql.js';
import { sendViewCartItemsEvent } from '../App/GTMEvents';

function useGtmData(products, isCartUpdating) {
    console.log("SKus cartItems = ", products);
    ...
    // Track the previous value of isCartUpdating
    const prevIsCartUpdating = usePrevious(isCartUpdating);
    ...
    useEffect(() => {
        ...
        if (!loading && !error && data) {
            ...
            if (data && data.getGtmViewCartItems.items.length > 0) {
                ...
                //console.log("Datalayer gtmViewCartItems = ", gtmViewCartItems);
                sendViewCartItemsEvent(gtmViewCartItems);
            }
        }
    }, [loading, data]);
