...
const Price = props => {
    ...
    const currencies = ['JOD', 'KWD', 'SAR', 'AED'];
    const isCurrencyCodeInArray = currencies.includes(currencyCode);

    const parts = patches.toParts.call(
        new Intl.NumberFormat(locale, {
            style: 'currency',
            currency: currencyCode,
            minimumFractionDigits: isCurrencyCodeInArray ? 2 : 0,
            maximumFractionDigits: isCurrencyCodeInArray ? 2 : 0,
        }),
        value
    );
