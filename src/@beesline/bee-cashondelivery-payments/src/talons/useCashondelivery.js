...
/**
 * Talon to handle cashondelivery payment.
 *
 ...
 * @param {DocumentNode} props.operations.getCashondeliveryConfigQuery query to fetch config from backend
 * @param {DocumentNode} props.operations.setPaymentMethodOnCartMutation mutation to set cashondelivery as payment
 *
 * @returns {
...
export const useCashondelivery = props => {
    ...
    const {
        getCashondeliveryConfigQuery,
        setPaymentMethodOnCartMutation
    } = operations;

    const [{ cartId }] = useCartContext();
    const { data } = useQuery(getCashondeliveryConfigQuery);
